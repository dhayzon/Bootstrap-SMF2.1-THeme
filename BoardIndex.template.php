...
function template_ic_block_recent()
{
	...
	elseif (!empty($context['latest_posts']))
	{
		...
		foreach ($context['latest_posts'] as $post)
			echo '<div class="my-1">
					 <a class="badge badge-primary" href="', $post['board']['href'], '">', $post['board']['name'], '</a>
					 ', $post['link'], ' ', $txt['by'], ' <strong>', $post['poster']['link'], '</strong><small class="float-right">  ', $post['time'], ' </small>
						 </div>';
		echo '
				</div>';
	}
