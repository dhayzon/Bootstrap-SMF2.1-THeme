...
function template_main()
{
	...
	echo '
		<div id="display_head" class="information">';
				// Next - Prev
		//https://stackoverflow.com/questions/37887065/php-regex-replace-all-text-between-a-tag
		$linkCustom = explode('-',$context['previous_next']);
		echo '<div class="nextlinks floatright">
					
						',preg_replace('/(<a.*?>).*?(<\/a>)/', '$1'.'	<span class="material-icons md-48">keyboard_arrow_left</span>'.'$2', $linkCustom[0]) ,'
					
						',preg_replace('/(<a.*?>).*?(<\/a>)/', '$1'.'	<span class="material-icons md-48">keyboard_arrow_right</span>'.'$2', $linkCustom[1]),'
		      </div>
				 ';
				 echo'
   ...
   //var_dump(explode('-',$context['previous_next']));
	...
	template_custom_single_post($message);
...
function template_single_post($message)
{
	...
	if ($message['can_approve'] || $message['can_unapprove'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'] || $context['can_quote'])
	{
		...
		if ($message['can_modify'])
			echo '
									<li class="quick_edit">
										<a title="', $txt['modify_msg'], '" class="modifybutton" id="modify_button_', $message['id'], '" onclick="oQuickModify.modifyMsg(\'', $message['id'], '\', \'', !empty($modSettings['toggle_subject']), '\')">', $txt['quick_edit'], '</a>
									</li>';
...
function template_custom_single_post($message){
	...
	if (!empty($message['is_ignored']))
	{
	 ...
	 $is_post = ($message['id'] == $context['topic_first_message'] ? true:false);
 
	// Show the message anchor and a "new" anchor if this message is new.
	echo '
				<div class="',$is_post? 'post':'comment  '.$message['css_class'].'','">
					', $message['id'] != $context['first_message'] ? '
					<a id="msg' . $message['id'] . '"></a>' . ($message['first_new'] ? '<a id="new"></a>' : '') : '', '
					<div class="post_wrapper">';
						...
						<div class="row no-gutters">';
							...
							if (!empty($modSettings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
								...
								<div class="avatar mt-1">
									<a href="', $message['member']['href'], '"><img class="rounded-circle" src="', $message['member']['avatar']['href'], '" width="32" height="32">  </a>
								</div>';

							echo'</div>';
							}
							echo'
							<div class="',$is_post? 'col-12':'col-11 pl-2 pl-md-0','">';
												 
							// Show a livk to the member's profile.
						 
							if($is_post){ 
								echo'<div class="post-autor mb-1">';
							// Show the user's avatar.
							if (!empty($modSettings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
								echo '
								<div class="avatar float-left mr-1 mt-1">
									<a href="', $message['member']['href'], '"><img class="rounded-circle" src="', $message['member']['avatar']['href'], '" width="32" height="32">  </a>
								</div>';
								echo'<div class="post-info"> 
												', $message['link'],'<br>
												', $message['member']['link'],' 	', $message['time'],' ', $txt['on'],' 
								...
								echo'								
								<div class="inner" data-msgid="', $message['id'], '" id="msg_', $message['id'], '"', $ignoring ? ' style="display:none;"' : '', '>
								',!$is_post? $message['member']['link'].': ':'','', $message['body'], '
								</div>
									...
									if (!empty($message['likes']['can_like']))
									{
										echo '
																<li class="smflikebutton" id="msg_', $message['id'], '_likes"', $ignoring ? ' style="display:none;"' : '', '>
																	<a href="', $scripturl, '?action=likes;ltype=msg;sa=like;like=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" class="msg_like"> ', $message['likes']['you'] ? $txt['unlike'] : $txt['like'], '</a>
																</li>';
									}
								...
								<a href="', $message['href'], '" rel="nofollow" title="', !empty($message['counter']) ? sprintf($txt['reply_number'], $message['counter'], ' - ') : '', $message['subject'], '" class="text-muted"><small>', $message['time'], '</small></a>
								  ';
								 ...
								 {
									 echo '
															 <ul class="quickbuttons float-right">';
									 ...
									 if ($message['can_modify'])
										 echo '
																 <li class="quick_edit">
																	 <a title="', $txt['modify_msg'], '" class="modifybutton text-muted" id="modify_button_', $message['id'], '" onclick="oQuickModify.modifyMsg(\'', $message['id'], '\', \'', !empty($modSettings['toggle_subject']), '\')"><small>', $txt['quick_edit'], '</small></a>
																 </li>';
							 
									 if ($message['can_approve'] || $message['can_unapprove'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'])
										 echo '
																 <li class="post_options"><small class="text-muted">', $txt['post_options'],'</small>';
